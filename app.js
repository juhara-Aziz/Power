...
function classify(r){players[0].hands=[];players[1].hands=[];for(const h of r?.landmarks||[]){const c=center(h),p=c.x<.5?players[0]:players[1];p.hands.push({center:c,open:openPalm(h)})}}
...
function draw(){ctx.clearRect(0,0,canvas.width,canvas.height);players.forEach(p=>{for(const h of p.hands){let x=h.center.x*canvas.width,y=h.center.y*canvas.height;ctx.strokeStyle=p.side?'#ff5a73aa':'#5d7cffaa';ctx.lineWidth=2;ctx.beginPath();ctx.arc(x,y,30,0,Math.PI*2);ctx.stroke()}
